...
<header>
	...
	<div class="right-header">
		...
		<h3>Chuyên mục Keyboard</h3>
	</div>
</header>
<body>

<?php 
	$sql = "SELECT * FROM `teach-keyboard` WHERE `accountid`=$accountid";
	$result = mysqli_query($conn, $sql);
	$row = mysqli_fetch_assoc($result);
	...
	echo '<tr><td>'.$row['question1'].'</td><td>'.$row['question2'].'</td><td>'.$row['question3'].'</td><td>'.$row['question4'].'</td><td>'.$row['question5'].'</td><td>'.$row['question6'].'</td><td>'.$row['question7'].'</td><td>'.$row['question8'].'</td><td>'.$row['question9'].'</td><td><a href="'.$link2.'" target="blank"><i class="far fa-eye"></i></a></td></tr></table>';
...
<div class="mark-point">
	<table>
		...
		<tr>
			<td>2</td>
			<td>Bấm các hợp âm căn bản</td>
		...
		<tr>
			<td>3</td>
			<td>Điệu thức các note trầm (hợp âm nền)</td>
		...
		<tr>
			<td>5</td>
			<td>Điệu thức các note cao (âm phụ)</td>
		...
		<tr>
			...
			<td>Tái hiện Melody đơn giản bằng tay phải</td>
		...
		<tr>
			<td>7</td>
			<td>Điệu thức kết hợp 2 tay</td>
		...
		<tr>
			<td>8</td>
			<td>Chơi melody có kèm hợp âm đệm</td>
		...
		<tr>
			<td>9</td>
			<td rowspan="3">Đọc bản nhạc</td>
			<td>Đọc note</td>
		...
		<tr>
			<td>15</td>
			<td rowspan="4">Sử dụng công cụ hỗ trợ</td>
			<td>Phần mềm/ thiết bị gõ nhịp</td>
		...
		<tr>
			<td>19</td>
			<td rowspan="4">Các yêu cầu đặc biệt</td>
...
<script type="text/javascript">
	...
	$('.mark').on('click',function(){
		...
		if(count==0){
			$.ajax({
				url: "http://localhost/Unisole/control-admin-views/AddPointKeyboard.php",
				type: "POST",
				dataType: "text",
				data: {
					accountid: a,
