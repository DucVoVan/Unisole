<?php 
	...
	$sql = "SELECT * FROM `teach-guitar` WHERE `status`=0";
	...
	while($row = mysqli_fetch_assoc($result)){
			...
			$link = "http://localhost/Unisole/views/PointGuitar.php?a=".$str."&&t=".$str2;
			if(empty($row['question6'])){ // Link file nhạc upload
				$link2 = "#";
			}else{
				$link2 = "http://localhost/Unisole/Controller-client/".$row['question6'];
				// Link file nhạc upload
			}
			echo "<tr>";
			echo "<td>";
			echo $row['question1'];
