<?php
	...
	$sql = "SELECT `accountid`,`question2` FROM `band` WHERE `status`=0";
	$result = mysqli_query($conn,$sql);
	echo '<h4>Đang chờ duyệt</h4>';
	echo '<table>';
	echo '<tr><th>STT</th><th>Tên band nhạc</th></tr>';
	$stt = 1;
	while($row = mysqli_fetch_assoc($result)){
		echo '<tr><td>'.$stt.'</td><td class="band" data-id="'.$row['accountid'].'">'.$row['question2'].'</td></tr>';
		$stt++;
	}
	echo "</table>";
?>
<script type="text/javascript">
	$('.band').click(function(){
			...
			$('.aprrove-band').append('<button class="btn btn-success ok-band">Phê duyệt</button>  ');
			$('.aprrove-band').append('<button class="btn btn-info info-band">Liên hệ</button>');

			$('.info-band').click(function(){
				$.ajax({
			...
			$('.ok-band').click(function(){
				$.ajax({
					url: "http://localhost/Unisole/control-admin-views/approvedBand.php",
					method: "POST",
					data: {
						id: $('input[name="accountid"]').val()
					},
		...
		$('.back-band-button').click(function(){
			$('.show-band-details').css("display","none");
			$('.ok-band').remove();
			$('.notok-band').remove();
			$('.info-band').remove();
