...
if(isset($_POST['phone'])){
	...
	if(mysqli_query( $conn, $sql1 )){
		...
		header("Location: http://localhost/Unisole/view-client/Enterphone.php");
	}
}else{
		...
		if(empty($row['phone'])){
			...
			if($courseregitered->checkCourse($row['id'])){
				...
				header("Location: http://localhost/Unisole/view-client/User.php");
					...
					// chưa có thì chuyển đến trang chọn khóa học
					// Kiểm tra xem người đã làm bản đánh giá năng lực hay chưa, gán giá trị vào biến Session
				$checkMusic = new CheckTopicCoursed($row['id']);
				$checkMusic->checkTopicCoursedMusic();
				$checkMusic->checkTopicCoursedGuitar();
				$checkMusic->checkTopicCoursedKeyboard();
