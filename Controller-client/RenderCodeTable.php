...
class RenderCodeTable{
    ...
    function render($topicchildren,$topicchildren_id,$start,$end,$arr_details,$arr){
        $rowspan = $end-$start+1;
        // echo $rowspan;
        ...
        for($i=$start; $i < $end+1; $i++){
            ...
            if($count == 1){// Nếu là cột đầu tiên được in ra, thì có thêm một cột td rowspan
                echo '<tr> <td>'.$i.'</td><td class="topicchildren" rowspan="'.$rowspan.'" data-id="'.$topicchildren_id.'">'.$topicchildren.'</td><td>'.$arr_details[$i].'</td><td>'.$arr[$key].'</td> <td>'.$arr[$key2].'</td> <td>'.$arr[$key3].'</td></tr>';
            }else{// Còn là cột thứ 2 hoặc 3, thì không có td rowspan
                echo '<tr> <td>'.$i.'</td><td>'.$arr_details[$i].'</td><td>'.$arr[$key].'</td> <td>'.$arr[$key2].'</td> <td>'.$arr[$key3].'</td></tr>';
            }
                $count++;
            }   
        }
